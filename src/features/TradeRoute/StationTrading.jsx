...
import { fetchOrdersByStation, fetchLocations } from '../../api/esiAPI';
import { flattenMarketTree } from '../../api/dataTransforms';
...
export default function StationTrading({ marketTree }) {
    ...
    const filteredStations = stationList.filter(s =>
        ...
        s.stationName.toLowerCase().includes(stationQuery.toLowerCase())
    );
                    ...
                    {stationQuery && filteredStations.length > 0 && (
                        <ul className="station-suggestions">
                            {filteredStations.slice(0, 5).map((s, idx) => (
                                <li
                                    key={idx}
                                    onClick={() => setStationQuery(s.stationName)}
                                    className="station-suggestion-item"
                                >
                                    {s.stationName} ({s.regionName})
                                </li>
                            ))}
                        </ul>
                    )}
