...
async function main() {
    ...
    for (const from of fromRegions) {
        ...
        for (const to of targets) {
            ...
            // Write primary hyphenated file variant
            const fileVariant = path.join(outDir, `${from}-${to}.json`);
            await fsp.writeFile(fileVariant, JSON.stringify(payload));
            console.log(`Wrote ${routes.length} routes to ${path.relative(repoRoot, fileVariant)}`);

            // Also write nested folder variant to match alternate client lookup paths
            const nestedDir = path.join(outDir, String(from));
            await fsp.mkdir(nestedDir, { recursive: true });
            const nestedVariant = path.join(nestedDir, `${to}.json`);
            await fsp.writeFile(nestedVariant, JSON.stringify(payload));
            console.log(`Wrote duplicate to ${path.relative(repoRoot, nestedVariant)}`);
        }
    }
}
